...
//////////////////////////////////////////////////////////////////////////
dz_float_t dz_math_absf( dz_float_t _value )
{
    return _value < 0.f ? -_value : _value;
}
//////////////////////////////////////////////////////////////////////////
dz_bool_t dz_math_is_finite( dz_float_t _value )
{
    return _value == _value && _value >= DZ_FLT_MIN && _value <= DZ_FLT_MAX ? DZ_TRUE : DZ_FALSE;
}
...
//////////////////////////////////////////////////////////////////////////
dz_float_t dz_math_length3( const dz_service_t * _service, dz_vec3_t _v )
{
    return DZ_SQRTF( _service, dz_math_dot3( _v, _v ) );
}
//////////////////////////////////////////////////////////////////////////
dz_vec3_t dz_math_normalize3( const dz_service_t * _service, dz_vec3_t _v, dz_vec3_t _fallback )
{
    const dz_float_t length = dz_math_length3( _service, _v );

    if( length <= 0.000001f || dz_math_is_finite( length ) == DZ_FALSE )
    {
        return _fallback;
    }
...
//////////////////////////////////////////////////////////////////////////
dz_quat_t dz_math_quat_normalize( const dz_service_t * _service, dz_quat_t _q )
{
    const dz_float_t length = DZ_SQRTF( _service, _q.x * _q.x + _q.y * _q.y + _q.z * _q.z + _q.w * _q.w );
...
{
    const dz_quat_t q = dz_math_quat_normalize( _service, *_quat );
    ...
    _euler->x = DZ_ATAN2F( _service, sinr, cosr ) * radians_to_degrees;
    _euler->y = ( dz_math_absf( sinp ) >= 1.f ? ( sinp < 0.f ? -DZ_PI_HALF : DZ_PI_HALF ) : DZ_ASINF( _service, sinp ) ) * radians_to_degrees;
    _euler->z = DZ_ATAN2F( _service, siny, cosy ) * radians_to_degrees;
}
//////////////////////////////////////////////////////////////////////////
void dz_quat_from_euler_xyz_degrees( const dz_service_t * _service, const dz_vec3_t * _euler, dz_quat_t * _quat )
{
    ...
    const dz_float_t cr = DZ_COSF( _service, _euler->x * degrees_to_half_radians );
    const dz_float_t sr = DZ_SINF( _service, _euler->x * degrees_to_half_radians );
    const dz_float_t cp = DZ_COSF( _service, _euler->y * degrees_to_half_radians );
    const dz_float_t sp = DZ_SINF( _service, _euler->y * degrees_to_half_radians );
    const dz_float_t cy = DZ_COSF( _service, _euler->z * degrees_to_half_radians );
    const dz_float_t sy = DZ_SINF( _service, _euler->z * degrees_to_half_radians );
...
//////////////////////////////////////////////////////////////////////////
dz_vec3_t dz_math_quat_rotate3( const dz_service_t * _service, dz_quat_t _q, dz_vec3_t _v )
{
    _q = dz_math_quat_normalize( _service, _q );
...
//////////////////////////////////////////////////////////////////////////
dz_vec3_t dz_math_transform_point( const dz_service_t * _service, const dz_transform_t * _transform, dz_vec3_t _point )
{
    dz_vec3_t scaled = { _point.x * _transform->scale.x, _point.y * _transform->scale.y, _point.z * _transform->scale.z };

    return dz_math_add3( dz_math_quat_rotate3( _service, _transform->rotation, scaled ), _transform->position );
}
//////////////////////////////////////////////////////////////////////////
dz_bool_t dz_math_is_finite3( dz_vec3_t _v )
{
    return dz_math_is_finite( _v.x ) == DZ_TRUE && dz_math_is_finite( _v.y ) == DZ_TRUE && dz_math_is_finite( _v.z ) == DZ_TRUE ? DZ_TRUE : DZ_FALSE;
}
//////////////////////////////////////////////////////////////////////////
void dz_mat4_identity( dz_mat4_t * _matrix )
{
    dz_memory_zero( _matrix->m, sizeof( _matrix->m ) );
    _matrix->m[0] = 1.f;
    _matrix->m[5] = 1.f;
    _matrix->m[10] = 1.f;
    _matrix->m[15] = 1.f;
...
{
    const dz_vec3_t forward = dz_math_normalize3( _service, _camera->forward, dz_math_vec3( 0.f, 0.f, -1.f ) );
    const dz_vec3_t right = dz_math_normalize3( _service, dz_math_cross3( forward, _camera->up ), dz_math_vec3( 1.f, 0.f, 0.f ) );
...
void dz_camera_test_aabb( const dz_service_t * _service, const dz_camera_state_t * _camera, const dz_aabb_t * _aabb, dz_bool_t * _visible )
{
    ...
    dz_camera_compute_view( _service, _camera, &view );
    dz_camera_compute_projection( _service, _camera, &projection );
